...
export default function DashboardPage() {
    ...
    const summary = useMemo(() => {
        const monthKeys = [...new Set(rows.map(r => r.date.slice(0, 7)))].sort().reverse();
    ...
    const openModalForDate = (date: string) => {
        ...
        setModalTitle(`รายละเอียดวันที่ ${format(new Date(date), 'dd MMM yyyy', { locale: th })}`);
        setModalData(data);
    };
    ...
    if (loading) {
        return (
            <main className="flex flex-col items-center justify-center h-screen text-blue-600 bg-slate-50">
                ...
                <p className="text-slate-500 font-medium animate-pulse">กำลังประมวลผล Dashboard...</p>
            </main>
        );
    }
            ...
            <main className="max-w-7xl mx-auto p-4 sm:p-6 mt-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
                    <div className="p-6 rounded-2xl shadow-sm bg-white border border-slate-100 flex flex-col justify-center relative overflow-hidden group hover:border-blue-200 transition-colors">
                        ...
                        <p className="text-sm font-medium text-slate-500 mb-1 z-10">จำนวนทริปทั้งหมด</p>
                        <p className="text-3xl font-bold text-blue-600 z-10">{rows.length.toLocaleString('th-TH')}</p>
                    </div>
                    ...
                    <div className="p-6 rounded-2xl shadow-sm bg-white border border-slate-100 flex flex-col justify-center relative overflow-hidden group hover:border-indigo-200 transition-colors">
                        ...
                        <p className="text-sm font-medium text-slate-500 mb-1 z-10">รวมระยะทาง (กม.)</p>
                    ...
                    <div className="p-6 rounded-2xl shadow-sm bg-white border border-slate-100 flex flex-col justify-center relative overflow-hidden group hover:border-emerald-200 transition-colors">
                        ...
                        <p className="text-sm font-medium text-slate-500 mb-1 z-10">เวลาการใช้งานรวม</p>
                    ...
                    <div className="p-6 rounded-2xl shadow-sm bg-gradient-to-br from-slate-800 to-slate-700 text-white flex flex-col justify-center relative overflow-hidden shadow-slate-200">
                        ...
                        <p className="text-sm font-medium opacity-80 mb-1 z-10">จำนวนรถในระบบ</p>
                        <p className="text-3xl font-bold z-10">{aggregated.length} <span className="text-lg font-normal opacity-80">คัน</span></p>
                    </div>
                </div>
                ...
                <div className="bg-white shadow-sm border border-slate-100 rounded-2xl p-6 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h2 className="font-semibold text-slate-800 text-lg">สถานะรถวันนี้</h2>
                        <p className="text-sm text-slate-500">{format(new Date(), 'EEEEที่ dd MMMM yyyy', { locale: th })}</p>
                    </div>
                    <div className="flex flex-wrap gap-3">
                        ...
                        <div className="bg-orange-50 text-orange-700 border border-orange-200 px-4 py-2 rounded-xl flex items-center gap-2">
                            ...
                            <span className="font-semibold">ถูกใช้งาน {busyCars.length} คัน</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                    <div className="bg-white p-6 shadow-sm border border-slate-100 rounded-2xl">
                        <h2 className="font-semibold mb-4 text-slate-800 text-lg">ระยะทางรวมแยกตามทะเบียน (กม.)</h2>
                    ...
                    <div className="bg-white p-6 shadow-sm border border-slate-100 rounded-2xl">
                        <h2 className="font-semibold mb-4 text-slate-800 text-lg">เวลาการใช้งานแยกตามทะเบียน (นาที)</h2>
                        <div className="h-72 w-full">
                            <Bar
                                data={{
                                    labels: aggregated.map(r => r.plate),
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="bg-white p-6 shadow-sm border border-slate-100 rounded-2xl flex flex-col">
                        <h2 className="font-semibold text-slate-800 text-lg mb-2">สัดส่วนทริปตามแผนก</h2>
                        <div className="flex-1 flex justify-center items-center h-72 w-full relative">
                            <Doughnut
                                data={{
                                    ...
                                    datasets: [{
                                        ...
                                        backgroundColor: ['#3b82f6', '#6366f1', '#8b5cf6', '#14b8a6', '#0ea5e9', '#cbd5e1'],
                                        borderWidth: 0,
                                        hoverOffset: 4,
                                    }]
                                }}
                                options={{
                                    ...
                                    plugins: {
                                        legend: { 
                                            position: 'bottom', 
                                            labels: { usePointStyle: true, padding: 20, font: { family: "'Prompt', sans-serif" } } 
                                        },
                                        tooltip: tooltipOptions
                                    }
                                }}
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
                                ...
                                <span className="text-xs text-slate-500 uppercase tracking-wider">ทริปรวม</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* LINE CHART */}
                    <div className="bg-white p-6 shadow-sm border border-slate-100 rounded-2xl">
                        <h2 className="font-semibold mb-4 text-slate-800 text-lg">แนวโน้มจำนวนทริปรายวัน</h2>
                        <div className="h-72 w-full">
                            <Line
                                data={{
                                    labels: Object.keys(tripsPerDay).map(dateStr => format(new Date(dateStr), 'dd MMM', { locale: th })),
                                    datasets: [{
                                        label: 'จำนวนทริป',
                                ...
                                options={{
                                    ...
                                    scales: {
                                        y: { beginAtZero: true, grid: { color: '#f1f5f9', drawBorder: false }, border: { display: false }, ticks: { stepSize: 1, padding: 10 } },
                ...
                <div className="bg-white p-6 shadow-sm border border-slate-100 rounded-2xl">
                    <h2 className="font-semibold mb-4 text-slate-800 text-lg">สรุปสถิติ 2 เดือนล่าสุด</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {summary.map((m) => {
                            ...
                            return (
                                <div key={m.month} className="bg-slate-50 border border-slate-100 rounded-xl p-5">
                                    <h3 className="font-bold text-lg text-indigo-700 mb-4 pb-2 border-b border-slate-200">
                                        เดือน {monthName}
                                    </h3>
                                    <div className="space-y-3 text-slate-700">
                                        <div className="flex justify-between items-center">
                                            <span className="flex items-center gap-2"><span className="p-1.5 bg-blue-100 text-blue-600 rounded-lg">🚘</span> จำนวนทริป</span>
                                            <span className="font-semibold">{m.trips} ครั้ง</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="flex items-center gap-2"><span className="p-1.5 bg-emerald-100 text-emerald-600 rounded-lg">🛣️</span> รวมระยะทาง</span>
                                            <span className="font-semibold">{m.km.toLocaleString('th-TH')} กม.</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="flex items-center gap-2"><span className="p-1.5 bg-orange-100 text-orange-600 rounded-lg">⏱️</span> เวลาใช้งาน</span>
                                            <span className="font-semibold">{formatDuration(m.mins)}</span>
                                        </div>
                                    </div>
                                </div>
                            );
                ...
                {/* MODAL (Drill Down) */}
                {modalData && (
                    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setModalData(null)}>
                        <div className="bg-white p-0 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                                <h3 className="text-lg font-bold text-slate-800">{modalTitle}</h3>
                                <button className="text-slate-400 hover:text-slate-600 p-1 bg-slate-100 rounded-full" onClick={() => setModalData(null)}>✕</button>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    <thead className="bg-slate-50 text-slate-500">
                                        <tr className="text-left">
                                            <th className="p-3 font-medium rounded-l-lg border-b border-slate-200">วันที่</th>
                                            <th className="p-3 font-medium border-b border-slate-200">แผนก</th>
                                            <th className="p-3 font-medium text-right border-b border-slate-200">ไมล์</th>
                                            <th className="p-3 font-medium text-right rounded-r-lg border-b border-slate-200">เวลา</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100">
                                        {modalData.map((r: any, i: number) => (
                                            <tr key={i} className="hover:bg-blue-50/50 transition-colors">
                                                <td className="p-3 text-slate-700">{format(new Date(r.date), 'dd MMM yy', { locale: th })}</td>
                                                <td className="p-3 text-slate-700 font-medium">{r.department}</td>
                                                <td className="p-3 text-right text-slate-600">{r.km.toLocaleString()} กม.</td>
                                                <td className="p-3 text-right text-slate-600">{formatDuration(r.mins)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
